    ...
    <footer className="border-t border-line py-16">
      <div className="mx-auto max-w-6xl px-6">
        <div className="grid gap-10 pb-12 md:grid-cols-[1.4fr,1fr,1fr,1fr]">
          <div>
            ...
            <p className="mt-4 max-w-xs text-sm text-muted">
              Automation that runs your busywork, so your team can do the work
              that matters.
            </p>
          </div>
          {cols.map((c) => (
            <div key={c.h}>
              ...
              <ul className="space-y-2 text-sm text-muted">
                {c.links.map((l) => (
                  <li key={l}><a href="#" className="transition-colors hover:text-fog">{l}</a></li>
                ))}
              </ul>
            </div>
          ))}
        ...
        <div className="flex flex-col items-center justify-between gap-3 border-t border-line pt-8 text-sm text-muted md:flex-row">
          ...
          <p>Designed &amp; built by <a href="https://seido-blond.vercel.app" className="text-fog hover:text-cyan">SEIDO</a></p>
        </div>
      </div>
    </footer>
  )
